...
/**
 * ONE ASSET, ONE REPORT — the shared decision surface.
 *
 * Rendered identically whether the data came from the committed daily
 * snapshot (the index ETFs) or from a live fetch triggered by search. That is
 * the entire point of extracting it: two layouts would drift, and a reader
 * comparing SPY to a searched ticker would be comparing two different
 * products rather than two assets.
 *
 * ── Reading order, which IS the design ────────────────────────────────
 *
 *   1. THE ANSWER   — bullish / bearish / wait / no edge, in one word.
 *   2. THE PLAN     — the levels, or the plain reason there are none.
 *   3. WHY          — what points up, what points down, what would change it.
 *   4. LEVELS       — where price sits against structure.
 *   5. THE WORKINGS — everything else, collapsed.
 *
 * The engine's self-assessment (evidence quality, agreement, per-module
 * detail) is deliberately LAST. A reader arriving here wants to decide, not
 * to audit; the audit is one click away and loses nothing by being there.
 *
 * This component COMPUTES NO OPINION. Every value is read off the bias and
 * plan it is handed; the only derivations are presentational.
 */
...
export function AssetReport({
  ...
  backHref: string;
  backLabel: string;
  /** What data backed this read. Rendered below the workings when supplied. */
  coverage?: React.ReactNode;
  footnote: string;
}) {
  const decision = data;
  const { bias, plan } = data;
  const tone = verdictTone(bias.verdict);

  /*
   * The headline answer, gated: a directional read whose plan was REFUSED
   * reads WAIT, never "bullish". See equityVerdict.ts.
   */
  const verdict = equityVerdict({
    bias,
    plan,
    refusal: data.planRefusal,
    earningsDate: data.earnings?.date ?? null,
  });
      ...
      <main className="mx-auto flex max-w-[1100px] flex-col gap-5 px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-baseline justify-between gap-3">
          <div className="flex items-baseline gap-3">
            <h1 className="font-mono text-lg font-semibold text-ink">{data.symbol}</h1>
            <span className="text-[11px] uppercase tracking-[0.16em] text-ink-faint">{data.name}</span>
            <span className="font-mono text-sm text-ink">{formatPrice(data.lastClose)}</span>
            <span
              className={`font-mono text-[11px] ${
                data.change24hPct > 0 ? "text-success" : data.change24hPct < 0 ? "text-danger" : "text-ink-faint"
              }`}
            >
              {data.change24hPct >= 0 ? "+" : ""}
          ...
          <Link href={backHref} className="text-[11px] uppercase tracking-[0.16em] text-ink-muted hover:text-ink">
            {backLabel}
          </Link>
        </div>
        ...
        <p className="text-[11px] text-ink-faint">{footnote}</p>
      </main>
    </div>
  );
}
