 ...
 * uses: the screen (IV / trailing RV at 21) and the forward ratio (IV /
 * forward RV). The realised premium is -ln(forwardRatio), so a forwardRatio
 * BELOW 1 is a win for the option buyer.
 */
const point = (
  date: string,
  symbol: string,
  ...
  ivTenorSessions: 21,
  trailing: [
    { windowSessions: 10, rvPct: 80 / screen, ratio: screen },
    { windowSessions: 21, rvPct: 80 / screen, ratio: screen },
    { windowSessions: 63, rvPct: 80 / screen, ratio: screen },
  ],
...
describe("the declaration itself", () => {
  ...
  it("pins one window, one sign and both gates before any data exists", () => {
    ...
    expect(d.primaryTrailingWindow).toBe(21);
